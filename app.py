...
# Pagination helper
def paginate(query, count_query, cur, page, per_page, params=None):
    if params is None:
        params = ()
    ...
    cur.execute(query + f" LIMIT {per_page} OFFSET {offset}", params)
    items = cur.fetchall()
...
def products():
    ...
    per_page = 10
    query = "SELECT p.*, s.name as supplier_name FROM products p LEFT JOIN suppliers s ON p.supplier_id = s.id ORDER BY p.id DESC"
    count_query = "SELECT COUNT(*) as total FROM products"
    products_list, page, total_pages, total = paginate(query, count_query, cur, page, per_page)
...
def suppliers():
    ...
    suppliers_list, page, total_pages, total = paginate(query, count_query, cur, page, per_page)
...
def transactions():
    ...
    page = request.args.get('page', 1, type=int)
    per_page = 15
    query = "SELECT t.*, p.name as product_name FROM transactions t LEFT JOIN products p ON t.product_id = p.id ORDER BY t.transaction_date DESC"
    count_query = "SELECT COUNT(*) as total FROM transactions"
    transactions_list, page, total_pages, total = paginate(query, count_query, cur, page, per_page)
    cur.execute("SELECT * FROM products")
    products_list = cur.fetchall()
    return render_template('transactions.html', transactions=transactions_list, products=products_list,
                           page=page, total_pages=total_pages, total=total)
...
def delete_staff(id):
    ...
    if user:
        ...
        db.commit()
        flash(f"{user['username']} has been removed successfully!")
    return redirect(url_for('manage_staff'))


@app.route('/suspend_staff/<int:id>', methods=['POST'])
def suspend_staff(id):
    ...
    if user:
        ...
        if new_status == 'suspended':
            flash(f"{user['username']} has been suspended successfully!")
        else:
